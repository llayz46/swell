...
class IssueDetailResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'comments' => $this->whenLoaded('comments', fn () => IssueCommentResource::collection(
                $this->comments->whereNull('parent_id')->sortBy('created_at')->values()
            )),
            'activities' => $this->whenLoaded('activities', fn () => IssueActivityResource::collection(
                $this->activities->sortByDesc('created_at')->values()
            )),
            'isSubscribed' => $this->when(
                isset($this->is_subscribed),
                fn () => $this->is_subscribed
            ),
            'subscribersCount' => $this->whenLoaded('subscribers', fn () => $this->subscribers->count()),
